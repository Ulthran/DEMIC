...
#' The main runner function for the DEMIC package
#'
#' @param bam_dir directory of input BAM files generated by bowtie2 (required)
#' @param fasta_dir directory of input FASTA files, each for a species (required)
#' @param out_dir directory of output files (default: ./)
#' @param window_size size (nt) of window for calculation of coverage (default: 5000)
#' @param window_step step (nt) of window for calculation of coverage (default: 100)
#' @param mapq_cutoff cutoff of mapping quality when calculating coverages (default: 5)
#' @param mapl_cutoff cutoff of mapping length when calculating coverages (default: 50)
#' @param max_mismatch_ratio maximum of mismatch ratio for each read as a hit (default: 0.03)
#' @param max_candidate_iter SOMETHING (default: 20)
#' @examples
#' demic("/input/bam_dir/", "/input/fa_dir/")
#' demic("/input/bam_dir/", "/input/fa_dir/", "/output/dir/")
demic <- function(bam_dir, fasta_dir, out_dir, window_size, window_step, mapq_cutoff, mapl_cutoff, max_mismatch_ratio, max_candidate_iter, max_threads){
  # Check parameters and fill in defaults
  if(missing(max_threads) || max_threads < 1){
    max_threads <- 1
  }

  stopifnot(! (missing(bam_dir) && missing(fasta_dir)))
  ...
  if(missing(out_dir)){
    out_dir <- "demic_output/"
  }
  if(! dir.exists(out_dir)){
    dir.create(out_dir, showWarnings = FALSE)
  }

  if(missing(window_step)){
    window_step <- 100
  }
  stopifnot(window_step > 10 && window_step < 1000)
  window_step <- as.integer(window_step)

  if(missing(window_size)){
    window_size <- 5000
  }
  stopifnot(window_size > 500 && window_size < 10000)
  window_size <- as.integer(window_size)

  if(window_size < 2 * window_step){
    window_size <- 2 * window_step
  }
  window_size <- as.integer(window_size/window_step) * window_step

  if(missing(mapq_cutoff)){
    mapq_cutoff <- 5
  }
  stopifnot(mapq_cutoff > 0 && mapq_cutoff < 30)

  if(missing(mapl_cutoff)){
    mapl_cutoff <- 50
  }
  stopifnot(mapl_cutoff > 30 && mapl_cutoff < 80)

  if(missing(max_mismatch_ratio)){
    max_mismatch_ratio <- 0.03
  }
  stopifnot(max_mismatch_ratio > 0.01 && max_mismatch_ratio < 0.3)

  # Check for bam files
  bam_files <- list.files(path = bam_dir, pattern = "\\.bam$")
  stopifnot(length(bam_files) != 0)
  if(length(bam_files) < max_threads){
    max_threads <- length(bam_files)
  }

  # Check for fasta files
  fa_files <- list.files(path = fasta_dir, pattern = "\\.fa$")
  fasta_files <- list.files(path = fasta_dir, pattern = "\\.fasta$")
  fna_files <- list.files(path = fasta_dir, pattern = "\\.fna$")
  FA_files <- list.files(path = fasta_dir, pattern = "\\.FA$")
  FASTA_files <- list.files(path = fasta_dir, pattern = "\\.FASTA$")
  FNA_files <- list.files(path = fasta_dir, pattern = "\\.FNA$")
  fasta_files <- c(fa_files, fasta_files, fna_files, FA_files, FASTA_files, FNA_files)
